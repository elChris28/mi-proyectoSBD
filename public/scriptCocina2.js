...
socket.emit("rolCocinero", "cocinero_2");
    ...
    : pedidos
        .map(pedido => `
          <div class="pedido">
            ...
            <button class="btn-listo" onclick="marcarComoListo('${pedido.id}', 'cocinero_2')">Listo</button>
          </div>
        `)
        .join("");
}
...
function renderizarPedidosListos(pedidos) {
        ...
        .map(
          (pedido, index) => `
            <div class="pedido listo">
              <strong>Mesa ${pedido.mesaId}</strong>
              <p>${pedido.platos.join(", ")}</p>
              <button class="btn-eliminar" onclick="eliminarPedidoListo(${index}, 'cocinero_2')">Eliminar</button>
            </div>
          `
        )
        .join("");
  ...
  if (pedidos.length > 0) {
    pedidosListosContainer.innerHTML += `
      <button class="btn-eliminar-todos" onclick="limpiarTodosLosPedidosListos('cocinero_2')">Limpiar todos</button>
    `;
  }
}
